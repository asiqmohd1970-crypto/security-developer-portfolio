...
export const Navbar: React.FC<NavbarProps> = ({ sequenceState }) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [flaskOnline, setFlaskOnline] = useState<boolean | null>(null);
  const [frameInfo, setFrameInfo] = useState<{ current: number; total: number }>({
    current: sequenceState?.currentFrame || 1,
    total: sequenceState?.totalFrames || 240,
  });

  // Listen for decoupled, zero-overhead frame updates from ScrollImageSequence
  useEffect(() => {
    const handleFrameUpdate = (e: Event) => {
      const customEv = e as CustomEvent<{ currentFrame: number; totalFrames: number }>;
      if (customEv.detail) {
        setFrameInfo({
          current: customEv.detail.currentFrame,
          total: customEv.detail.totalFrames,
        });
      }
    };
    window.addEventListener('scroll-frame-change', handleFrameUpdate);
    return () => window.removeEventListener('scroll-frame-change', handleFrameUpdate);
  }, []);
  ...
  return (
    <header
      id="main-navbar"
      className="fixed top-0 left-0 right-0 z-50 px-4 sm:px-5 md:px-10 py-3.5 backdrop-blur-[8px] bg-black/20 border-b border-white/[0.08] transition-all duration-300"
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] text-[11px] font-mono text-slate-300">
            ...
            <span>
              {String(frameInfo.current).padStart(3, '0')}
              <span className="text-slate-500">/{frameInfo.total}</span>
            </span>
          </div>
